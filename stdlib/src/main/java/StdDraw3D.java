...
public final class StdDraw3D implements MouseListener, MouseMotionListener, MouseWheelListener, KeyListener, ActionListener, ChangeListener, ComponentListener,
	WindowFocusListener
{
	...
	private static JRadioButtonMenuItem orbitModeButton, fpsModeButton, airplaneModeButton, lookModeButton, fixedModeButton;
	...
	private static void initializeCanvas()
	{
		...
		canvas = new Canvas3D(config)
		{
			...
			public void postRender()
			{
				...
				/*
				 * while (!showedOnce) { try { Thread.currentThread().sleep(0);
			...
			/*
			 * public void postSwap () { while (!showedOnce) { try {
			 * Thread.currentThread().sleep(5); } catch (InterruptedException e)
			 * { System.out.println("Error sleeping"); } } //if (!showedOnce) {
			 * // try { Thread.currentThread().sleep(30); } // catch
			 * (InterruptedException e) { System.out.println("Error sleeping");
			 * } //} //StdOut.println("showed once is false, rendered once");
			 * showedOnce = false; //renderedOnce = true; //Thread.yield(); }
			 */
		};
	...
	private static JMenuBar createMenuBar()
	{
		...
		loadButton.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_L, Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()));
		...
		save3DButton.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_E, Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()));
		...
		quitButton.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_Q, Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()));
		...
		orbitModeButton.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_1, Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()));
		...
		fpsModeButton.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_2, Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()));
		...
		airplaneModeButton.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_3, Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()));
		...
		lookModeButton.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_4, Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()));
		...
		fixedModeButton.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_5, Toolkit.getDefaultToolkit().getMenuShortcutKeyMask()));
	...
	public static void setBackgroundSphere(String imageURL)
	{
		...
		Sphere sphere = new Sphere(1.1f, Sphere.GENERATE_NORMALS | Sphere.GENERATE_NORMALS_INWARD | Sphere.GENERATE_TEXTURE_COORDS, numDivisions);
	...
	public static Shape box(double x, double y, double z, double w, double h, double d, double xA, double yA, double zA, String imageURL)
	{
		...
		com.sun.j3d.utils.geometry.Box box = new com.sun.j3d.utils.geometry.Box(dimensions.x, dimensions.y, dimensions.z, PRIMFLAGS, ap, numDivisions);
	...
	public static Shape lines(double[] x, double[] y, double[] z, Color[] colors)
	{
		...
		GeometryArray geom = new LineStripArray(coords.length, LineArray.COORDINATES | LineArray.COLOR_4, new int[] { coords.length });
		geom.setCoordinates(0, coords);
	...
	public static void overlayPicture(double x, double y, String s, double w, double h)
	{
		...
		{
			getGraphics2D(offscreenImage).drawImage(image, (int) Math.round(xs - ws / 2.0), (int) Math.round(ys - hs / 2.0), (int) Math.round(ws),
	...
	public static void save(String filename)
	{
		...
		javax.media.j3d.Raster ras = new javax.media.j3d.Raster(new Point3f(-1.0f, -1.0f, -1.0f), javax.media.j3d.Raster.RASTER_COLOR, 0, 0, width, height,
			imageComp, null);
	...
	public static class Vector3D
	{
		...
		// --------------------------------------------------------------------------
		/*
		 * public Vector3D (double min, double max) { this.x = min +
		 * Math.random() * (max - min); this.y = min + Math.random() * (max -
		 * min); this.z = min + Math.random() * (max - min); }
		 */
		// --------------------------------------------------------------------------
		/**
		 ...
		 */
		/*
		 * public void drawLine (Vector3D origin) { Vector3D end =
		 * this.plus(origin); StdDraw3D.line(origin.x, origin.y, origin.z,
		 * end.x, end.y, end.z); }
		 */
	}
